...
export class ModifyPasswordComponent implements OnInit {
  ...
  onSubmit() {
    if (this.formModify.valid) {
      ...
      this.authService.newPassword(password, this.token).subscribe(this.success, this.error);
    }
  }
  ...
  verificarInput(value: string) {
    return (
      this.formModify.get(value)?.invalid && (this.formModify.get(value)?.dirty || this.formModify.get(value)?.touched)
    );
  }
}
